...
task clock_driver::drive(); 
   `uvm_info(get_name(), "drive starting...", UVM_LOW)
   seq_item_port.get_next_item(req);
   `uvm_info(get_name(), "get_next_item done", UVM_LOW)
   `uvm_info(get_name(), req.sprint(),UVM_MEDIUM)
   ...
   `uvm_info(get_name(), "Start driving clock signal.", UVM_LOW)
   clk_if.clk <= clk_init;
   #(clk_skew);
   ...
   fork 
     ...
     forever begin
          #500000;  
          `uvm_info(get_name(), "clocking......", UVM_MEDIUM)
        end
   join_any
endtask: drive
