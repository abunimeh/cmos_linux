...
def gen_env_dic(env_iter):
    ged = {}
    for env in env_iter:
        env_str = os.environ.get(env)
        if not env_str:
            raise Exception("env {0} is NA".format(env))
        ged[env] = env_str
    return ged
...
def format_cfg_str(cfg_str):
    return [cc.strip() for cc in re.split(r',|{0}'.format(os.linesep),
                                          cfg_str) if cc]

def find_iter(path, pattern, dir_flg=False, cur_flg=False):
    if cur_flg:
