...
class TestGenEnvDic(unittest.TestCase):
    def test_gen_env_dic(self):
        env_lst = ['USER', 'HOME']
        ged = pcom.gen_env_dic(env_lst)
        for env_item in env_lst:
            self.assertEqual(ged[env_item], os.environ.get(env_item))

class TestGenCfg(unittest.TestCase):
    def setUp(self):
        self.tmp_cfg1_file = '/tmp/test_prun1.cfg'
        self.tmp_cfg2_file = '/tmp/test_prun2.cfg'
        cfg1_str = '''\
[section1]
key1 = val1, val2
key2 = val3,
