...
var a = 0,
  ...
  is_submission = false,
  soft_sub = false,
  // formula_display = jQuery('#formula')
...
function reset_calc() {
  ...
  //reset formula table to initial
  $("table.order-list tbody tr").filter(function(index) {
    if(index != 0) {
      $(this).remove();
    } else {
      $(this).children().find('input[name="fml"]').val("");
    }
  });
  del_formula_input();
  //initial coulmn
...
//brackets
jQuery('#left_brkt, #right_brkt').each(function() {
  jQuery(this).click(function() {
    var value = jQuery(this).val();
    $('table.order-list input[name^="fml"]').each(function(i, item){
        if($(item).attr("class") == "formula-input"){
