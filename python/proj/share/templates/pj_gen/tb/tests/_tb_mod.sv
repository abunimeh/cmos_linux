...
{%-for a_name in agt_name_lst%}
      uvm_config_db #({{a_name}}_if)::set(null,"uvm_test_top.env.{{a_name}}_agt.{{a_name}}_drv","{{a_name}}_drv_if",test_top.{{a_name}}_if); 
      uvm_config_db #({{a_name}}_if)::set(null,"uvm_test_top.env.{{a_name}}_agt.mon","{{a_name}}_mon_if",test_top.{{a_name}}_if); 
{% endfor %}
