...
(function(win) {
	...
	*/
    ...
    //初始化日期
    jeDt.nowDate = function(timestamp, format, diff) {
        var De = new Date(timestamp | 0 ? function(tamp) {
            return tamp < 864e5 ? +new Date() + tamp * 864e5 :tamp;
        }(parseInt(timestamp)) :+new Date());
        return jeDt.parse([ De.getFullYear(), De.getMonth() + 1, De.getDate() - diff], [ De.getHours(), De.getMinutes(), De.getSeconds() ], format);
    };
    ...
    InitDate.prototype = {
        init:function() {
            ...
            var nowDateVal = jeDt.nowDate(null, opts.format, opts.date_diff);
            if (opts.isinitVal) {
        ...
        setHtml:function(opts, self) {
            var that = this, weekHtml = "", date = new Date(), nowDateVal = jeDt.nowDate(null, opts.format, opts.date_diff), isformat = opts.format.match(/\w+|d+/g).join("-") == "YYYY-MM" ? true :false;
